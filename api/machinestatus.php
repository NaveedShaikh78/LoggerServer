...
$sql="select IFNULL(COUNT(machinelog.end_time),0) as count,machinestatus.ioport,machinestatus.statetime,machinestatus.status,machinestatus.opid,machinestatus.jobid from machinestatus left join machinelog on machinestatus.ioport = machinelog.ioport and machinelog.start_time between ".$condition." and TIMESTAMPDIFF(SECOND,machinelog.start_time,machinelog.end_time)>20  group by  machinestatus.ioport,machinestatus.statetime,machinestatus.status";
